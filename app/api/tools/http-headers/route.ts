...
import { runHTTPHeaders } from "@/lib/utils/security-tools"

async function httpHeadersHandler(req: NextRequest) {
  try {
    await connectDB()
    ...
    const result = await runHTTPHeaders(url)
...
export const POST = withAuth(httpHeadersHandler)
